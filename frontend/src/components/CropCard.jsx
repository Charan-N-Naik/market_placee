    ...
    <>
      ...
      >
        ...
        <div className="relative h-72 sm:h-80 w-full overflow-hidden bg-gray-100">
          ...
          {/* Organic Tag */}
          {listing.isOrganic && (
            <div className="absolute top-4 left-4 bg-[#1F7A4D] text-white text-xs font-black px-4 py-2 rounded-full shadow-lg uppercase tracking-wider flex items-center gap-2">
              <Leaf size={14} /> Organic <span className="text-sm">🌿</span>
            </div>
          )}

          {/* Multiple Image Count Badge */}
          <div className="absolute bottom-4 left-4 bg-black/70 backdrop-blur-md text-white text-xs font-extrabold px-3.5 py-1.5 rounded-full flex items-center gap-2 border border-white/20 shadow-md">
          ...
          <div className="absolute top-4 right-4 flex items-center gap-2.5 z-20">
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-white/95 backdrop-blur-md hover:bg-white text-gray-800 flex items-center justify-center shadow-lg transition-transform hover:scale-110 cursor-pointer border border-gray-100"
              title="Quick Preview"
            >
              <Eye size={18} />
            </button>
            ...
            <button
              onClick={handleShare}
              className="w-10 h-10 rounded-full bg-white/95 backdrop-blur-md hover:bg-white text-gray-800 flex items-center justify-center shadow-lg transition-transform hover:scale-110 cursor-pointer relative border border-gray-100"
              title="Share Crop"
            >
              {copiedShare ? <Check size={18} className="text-[#1F7A4D]" /> : <Share2 size={18} />}
            </button>

            {/* Wishlist Heart Button */}
            <button
              onClick={handleSaveToggle}
              className={`w-10 h-10 rounded-full backdrop-blur-md flex items-center justify-center shadow-lg transition-transform hover:scale-110 cursor-pointer border border-gray-100
            ...
            >
              <Heart size={18} fill={isSavedLocal ? 'currentColor' : 'none'} />
            </button>
          </div>
        </div>

        {/* Card Body — 3D Luxury Marketplace Card */}
        <div className="p-6 sm:p-7 md:p-8 flex-1 flex flex-col justify-between space-y-6">
          
          {/* CROP INFORMATION */}
          <div className="space-y-2">
            <span className="text-xs font-bold text-[#1F7A4D] uppercase tracking-wider block truncate">
              {categoryName} • {varietyName}
            </span>
            <h3 className="text-2xl md:text-3xl font-black text-gray-900 leading-tight group-hover:text-[#1F7A4D] transition-colors flex items-center gap-2.5">
          ...
          <div className="py-4 border-y-2 border-gray-100 flex items-center justify-between gap-4">
            <div>
              <div className="flex items-baseline gap-2 flex-wrap">
                <span className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900">₹{price}</span>
                <span className="text-xs md:text-sm text-gray-500 font-bold">/ {unit}</span>
                <span className="text-xs text-gray-400 line-through font-semibold">₹{previousPrice}</span>
              </div>
              <p className="text-xs font-bold text-[#FF8C42] mt-1.5 leading-snug">
                {discountPercent}% OFF • Available Stock: {stockQuantity} {unit}
              </p>
            </div>

            <span className="px-3.5 py-2 bg-[#E8F7EE] text-[#1F7A4D] text-xs font-black rounded-xl border border-[#1F7A4D]/25 shadow-xs shrink-0">
              {reportData.grade}
            </span>
          </div>
          ...
          <div className="space-y-3 pt-1">
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <button
                ...
                disabled={adding}
                className={`min-h-[48px] px-3 sm:px-4 py-3 rounded-2xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm active:scale-95 whitespace-nowrap overflow-hidden
                  ${added 
              ...
              <button
                ...
                className="min-h-[48px] px-3 sm:px-4 py-3 bg-[#FF8C42] hover:bg-[#e07530] text-white rounded-2xl font-black text-xs sm:text-sm uppercase tracking-wide shadow-md shadow-orange-500/20 transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-95 whitespace-nowrap overflow-hidden border-2 border-[#FF8C42]"
              >
            ...
            <button
              onClick={handleFullView}
              className="w-full min-h-[48px] px-4 py-3.5 bg-gray-900 hover:bg-black text-white rounded-2xl font-extrabold text-xs sm:text-sm uppercase tracking-wider shadow-md transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-95 whitespace-nowrap overflow-hidden border-2 border-gray-900"
            >
